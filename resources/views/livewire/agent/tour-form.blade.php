    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <h2 class="text-2xl font-bold mb-6">{{ $tour ? 'Edit Tour' : 'Create New Tour' }}</h2>
                ...
                <form wire:submit="save" class="space-y-6">
                    ...
                    <div class="border p-4 rounded-md bg-gray-50 dark:bg-gray-700">
                        ...
                        @foreach($destinations as $index => $destination)
                            <div class="flex gap-4 mb-2 items-start">
                                <div class="flex-1">
                                    <input type="text" wire:model="destinations.{{ $index }}.city"
                                        placeholder="City (e.g. Hunza)"
                                        class="w-full rounded-md border-gray-300 dark:bg-gray-900">
                                    @error("destinations.$index.city") <span
                                    class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div class="flex-1">
                                    <input type="text" wire:model="destinations.{{ $index }}.special_notes"
                                        placeholder="Special Notes (Optional)"
                                        class="w-full rounded-md border-gray-300 dark:bg-gray-900">
                                </div>
                                <button type="button" wire:click="removeDestination({{ $index }})"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2">
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Duration: <span class="font-bold text-blue-600">{{ $duration_days }} Days</span>
                            </p>
                        </div>
                        <div>
                            ...
                            <input type="number" wire:model="price_per_person"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900">
                            @error('price_per_person') <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                    ...
                    <div class="border p-4 rounded-md bg-gray-50 dark:bg-gray-700">
                        ...
                        <div class="mb-4">
                            ...
                            @foreach($site_visits as $index => $visit)
                                <div class="flex gap-4 mb-2 items-start">
                                    <input type="text" wire:model="site_visits.{{ $index }}.name" placeholder="Site Name"
                                        class="flex-1 rounded-md border-gray-300 dark:bg-gray-900">
                                    <input type="number" wire:model="site_visits.{{ $index }}.price"
                                        placeholder="Price (Optional)"
                                        class="w-32 rounded-md border-gray-300 dark:bg-gray-900">
                                    <button type="button" wire:click="removeSiteVisit({{ $index }})"
                                        class="text-red-500 hover:text-red-700 px-2">✕</button>
                                </div>
                            @endforeach
                        ...
                        <div>
                            ...
                            @foreach($custom_addons as $index => $addon)
                                <div class="flex gap-4 mb-2 items-start">
                                    <input type="text" wire:model="custom_addons.{{ $index }}.name"
                                        placeholder="Add-on Name"
                                        class="flex-1 rounded-md border-gray-300 dark:bg-gray-900">
                                    <input type="number" wire:model="custom_addons.{{ $index }}.price"
                                        placeholder="Price (Optional)"
                                        class="w-32 rounded-md border-gray-300 dark:bg-gray-900">
                                    <button type="button" wire:click="removeCustomAddon({{ $index }})"
                                        class="text-red-500 hover:text-red-700 px-2">✕</button>
                                </div>
                            @endforeach
                    ...
                    <div>
                        ...
                        @error('temporaryImages.*') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        @error('temporaryImages') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
