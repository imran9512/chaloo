...
<section>
    ...
    @if(auth()->user()->isTransporter() || auth()->user()->isAgent())
        <div class="mt-10 pt-10 border-t border-gray-200 dark:border-gray-700">
            <header>
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {{ __("These documents are used for account verification and cannot be changed.") }}
                </p>
            </header>
            ...
            <div class="mt-6 grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if(auth()->user()->id_card_image)
                        <x-lightbox-image :src="route('storage.serve', ['path' => auth()->user()->id_card_image], false)"
                            alt="ID Card Front" />
                    @else
                ...
                <div>
                    ...
                    @if(auth()->user()->id_card_back_image)
                        <x-lightbox-image :src="route('storage.serve', ['path' => auth()->user()->id_card_back_image], false)"
                            alt="ID Card Back" />
                    @else
                        <span class="text-red-500 text-xs">Missing</span>
                    @endif
                ...
                <div>
                    ...
                    @if(auth()->user()->license_image)
                        <x-lightbox-image :src="route('storage.serve', ['path' => auth()->user()->license_image], false)"
                            alt="License Front" />
                    @else
                        <span class="text-red-500 text-xs">Missing</span>
                    @endif
                ...
                <div>
                    ...
                    @if(auth()->user()->license_back_image)
                        <x-lightbox-image :src="route('storage.serve', ['path' => auth()->user()->license_back_image], false)"
                            alt="License Back" />
                    @else
                        <span class="text-red-500 text-xs">Missing</span>
                    @endif
